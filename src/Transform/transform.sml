		...
		fun writeTransformFunction indent =
		  let
		    fun printActions indent = 
		      let
			fun printPatternMatch (comment, matchArgs) =
			  let fun printComment (comment) = (* Note trailing indent *)
				print ((WriteUtil.writeComment indent comment)^"\n"^indent)
			      fun printMatch(pat, action, SOME ()) = (print "|"; printMatch(pat, action, NONE))
				| printMatch(pat, action, NONE) =
				let val indentedAction = WriteUtil.stringIndent (action) (indent^"  ")
				 in print (pat ^ "=>\n" ^ indentedAction ^ "\n")
			        end
			  in (printComment(comment); printMatch matchArgs)
			 end
			fun printAction (i, action, firstOpt) =
			  SOME(printPatternMatch ("generated from pattern: " ^ Vector.sub(patterns, i),
				(Int.toString i, action, firstOpt)))
			val defaultOnly = case (Vector.length actions) of 0 => NONE | _ => SOME();
		      in (Vector.foldli (printAction) NONE actions
			  ;case default of
			    SOME d => printPatternMatch ("generated for the default case", ("_", d, defaultOnly))
			  | NONE => printPatternMatch ("generated by default  for the default case",
						       ("_", Params.defaultAction, defaultOnly)))
		      end
